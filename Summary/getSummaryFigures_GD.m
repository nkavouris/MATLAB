...
for k=1:length(structure)
    ...
    grid on
    %% TTT CTC vs OOB
    y=[mean(X4TTTsm) mean(X6TTTsm)-.81; mean(X4TTTro) mean(X6TTTro); mean(X4TTTse) mean(X6TTTse)];
    y=y';
    x=categorical({'Smoking'; 'Roasting';'Searing'});
    ...
    legend('X4',"X6",'Location','southoutside','Orientation','horizontal')
    % TTT OOB
    y=[mean(X4TTTsm) mean(X6TTTsm);  mean(X6TTTro); mean(X4TTTse) mean(X6TTTse);
    y=y';
    x=categorical({'Smoking'; 'Roasting';'Searing'});
    ...
    legend('X4',"X6",'Location','southoutside','Orientation','horizontal')
    %% TTT by Start Temp
    y=[mean(coldSmoking) mean(coldRoasting) mean(coldSearing); mean(warmSmoking) mean(warmRoasting) mean(warmSearing); mean(hotSmoking) mean(hotRoasting) mean(hotSearing)];
